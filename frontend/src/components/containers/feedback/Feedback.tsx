...
import { IconCheck, IconChevronLeft, IconChevronRight, IconStar, IconX } from '@/components/ui/icons';
...
const Feedback: React.FC = () => {
  ...
  const [submitState, setSubmitState] = useState<{ type: 'idle' } | { type: 'success' } | { type: 'error'; message: string }>({ type: 'idle' });
      ...
      <div className="flex gap-1">
        {Array.from({ length: 5 }).map((_, i) => {
          ...
          return interactive && setRating ? (
            ...
            <IconStar key={`s-${i}`} filled={filled} size={18} className={filled ? 'text-brand-primary' : 'text-sand-300'} />
          );
        })}
      </div>
    );
  ...
  const effectiveIdx = slides.length ? ((activeIdx % slides.length) + slides.length) % slides.length : 0;
                    ...
                    <div className="flex flex-wrap gap-6 items-center">
                        <div className="flex gap-3">
                            <button
                                ...
                                aria-label={t.prev}
                                type="button"
                                onClick={() => setActiveIdx((p) => (p - 1 + (slides.length || 1)) % (slides.length || 1))}
                ...
                <form onSubmit={submitReview} className="space-y-5">
                                    <div className="grid grid-cols-1 gap-5">
                                        <div className="space-y-1">
                                            <label htmlFor="feedback-name" className="text-sm font-semibold text-text-primary">{t.fName}</label>
                                        <input 
                                            id="feedback-name"
                                            type="text" 
                                            required
                                    ...
                                    <div className="space-y-1">
                                        <label htmlFor="feedback-email" className="text-sm font-semibold text-text-primary">{t.fEmail}</label>
                                        <input 
                                            id="feedback-email"
                                            type="email" 
                                            required
                                    ...
                                    <div className="space-y-1">
                                        <label htmlFor="feedback-comment" className="text-sm font-semibold text-text-primary">{t.fComment}</label>
                                        <textarea
                                            id="feedback-comment"
                                            rows={4}
                                            required
