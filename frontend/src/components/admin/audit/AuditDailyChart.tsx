...
export type AuditDailyRow = {
  date: string; // expected YYYY-MM-DD (but we normalize)
  requests: number;
  unique_ips: number;
  errors: number;
};
    ...
    <div>
      <div className="d-flex flex-wrap gap-3 align-items-center mb-2">
        ...
        <div className="ms-auto d-flex gap-3 align-items-center">
          ...
          {loading && <span className="text-muted small">Yükleniyor...</span>}
        </div>
      </div>
      ...
      <div className="border rounded bg-white" style={{ overflow: 'hidden' }}>
        <svg
          viewBox={`0 0 ${W} ${H}`}
          width="100%"
          height={H}
        ...
        {hasAny && (
          <div className="px-3 py-2 border-top d-flex justify-content-between small text-muted">
            <div>{data[data.length - 1]?.date ? fmtIsoNice(data[data.length - 1].date) : '-'}</div>
            <div className="d-flex gap-3">
              <span>
                requests:{' '}
                <strong className="text-dark">{data[data.length - 1]?.requests ?? 0}</strong>
              </span>
              {showUnique && (
                <span>
                  unique:{' '}
                  <strong className="text-dark">{data[data.length - 1]?.unique_ips ?? 0}</strong>
                </span>
              )}
              {showErrors && (
                <span>
                  errors:{' '}
                  <strong className="text-dark">{data[data.length - 1]?.errors ?? 0}</strong>
                </span>
              )}
            </div>
          </div>
