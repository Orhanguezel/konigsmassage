...
import { services, servicesI18n } from '@vps/shared-backend/modules/services/schema';
...
import { getPaymentConfig } from '@vps/shared-backend/modules/siteSettings/service';
...
export const getBookingPaymentInfo: RouteHandler = async (req, reply) => {
  ...
  // Get service price — fiyat parent `services` tablosunda (locale-bagimsiz),
  // isim ise services_i18n'de (locale bazli, 'de' fallback'li).
  ...
  if (booking.service_id) {
    const [svc] = await db
      .select({ price_onetime: services.price_onetime, currency: services.currency })
      .from(services)
      .where(eq(services.id, booking.service_id))
      .limit(1);
    servicePrice = svc?.price_onetime ? Number(svc.price_onetime) : 0;
    priceDisplay = servicePrice ? `${servicePrice.toFixed(2)} ${svc?.currency || 'EUR'}` : '';

    const [i18n] = await db
      .select({ name: servicesI18n.name })
      .from(servicesI18n)
      .where(and(eq(servicesI18n.service_id, booking.service_id), eq(servicesI18n.locale, locale)))
      .limit(1);
    serviceName = i18n?.name || '';

    if (!serviceName) {
      const [i18nDe] = await db
        .select({ name: servicesI18n.name })
        .from(servicesI18n)
        .where(and(eq(servicesI18n.service_id, booking.service_id), eq(servicesI18n.locale, 'de')))
        .limit(1);
      serviceName = i18nDe?.name || '';
    }
  }
...
export const payBooking: RouteHandler = async (req, reply) => {
  ...
  if (booking.service_id) {
    const [svc] = await db
      .select({ price_onetime: services.price_onetime })
      .from(services)
      .where(eq(services.id, booking.service_id))
      .limit(1);
    servicePrice = svc?.price_onetime ? Number(svc.price_onetime) : 0;
