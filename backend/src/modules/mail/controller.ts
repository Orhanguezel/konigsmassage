...
export const sendTestMail: RouteHandler = async (req, reply) => {
  try {
    ...
    await sendMailRaw({
      to,
      subject: 'SMTP Test – konigsmassage',
    ...
    req.log.error({ err: e, code: e?.code, response: e?.response }, 'smtp_test_failed');
...
export const sendMailHandler: RouteHandler = async (req, reply) => {
  try {
    ...
    req.log.error(e);
    return reply.code(500).send({ error: { message: 'mail_send_failed', details: e?.message } });
  }
};
